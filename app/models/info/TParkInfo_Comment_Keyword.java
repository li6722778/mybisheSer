...
public class TParkInfo_Comment_Keyword extends Model {
	 ...
	 * 
	 关键字搜索
	 */
	public static Page<TParkInfo_Comment_Keyword> pageByFilter(int currentPage,
		...
		Page<TParkInfo_Comment_Keyword> allData = elist.orderBy(orderBy)
	 ...
	 * @return
	 */
	public static boolean inkeywords(String content) {
		ExpressionList<TParkInfo_Comment_Keyword> elist = find.where();
		elist.like("content", "%"+content+"%");
		int count = elist.findRowCount();
		if (count > 0) {
			return true;
		} else {
			return false;
