...
import models.info.TPKGenerator;
...
public class UserController extends Controller {
	...
	public static Result getDataById(Long userid) {
		...
		String json = gsonBuilderWithExpose.toJson(TuserInfo
				.findDataById(userid));
	...
	public static Result saveData() {
		...
		TuserInfo user = gsonBuilderWithExpose.fromJson(request,
				TuserInfo.class);
	...
	public static Result changePasswdData() {
		...
		try {
			...
			try {
				if (!Crypto.decryptAES(user.updatePerson).equals(
						currentUser.passwd)) {
	...
	public static Result regUser(String code) {
		...
		final TuserInfo user = gsonBuilderWithExpose.fromJson(request,
				TuserInfo.class);
		...
		try {
			...
			if (verficode == null || verficode.verifycode == null
					|| !verficode.verifycode.trim().equals(code.trim())) {
				throw new Exception("注册验证码有误,请重新获取");
			} else {
				// 有效的情况下
				TVerifyCode.deletePhone(user.userPhone);
				// 三分钟有效用存储过程来删除？？
				Logger.info("verified code successfully,delete verification code "
						+ verficode.verifycode);
			}
			// 检查完毕
	...
	public static Result getAdmDataById(Long userid) {
		Logger.info("start to get data");
		String json = gsonBuilderWithExpose.toJson(TParkInfo_adm
				.findAdmPartInfo(userid));
		JsonNode jsonNode = Json.parse(json);
		Logger.debug("got Data:" + json);
		return ok(jsonNode);
	}
