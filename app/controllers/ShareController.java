...
public class ShareController extends Controller {
	...
	public static Result sendShareById(final Long telephonenumber, Long url) {
		...
		{	
		 ...
		 //该url链接已经被使用，查询该手机号是否在该url中记录过
			...
			//手机号不在该url的分享记录中
			if(result==false) {
			boolean  sendreuslt =sendtouser(telephonenumber, uniqueurl, times, counponcodes);
				if(sendreuslt==true)
				{
					response.setResponseStatus(ComResponse.STATUS_OK);	}
				else {
	...
	public static void sendrandomshareCounpon(String[] counponcodes,Long id) {
	 ...
	 * @param uniqueurl
	 */
	public static boolean sendtouser(Long telephonenumber,Tuniqueurl uniqueurl,int times, String[] counponcodes){
			// 证明该链接的分享获取优惠劵次数未用完
			if (uniqueurl.sharetime < times) {
				...
				if (userInfo != null) {
					// 随机赠送优惠劵
					sendrandomshareCounpon(counponcodes,userInfo.userid);
					// url的分享次数加一
